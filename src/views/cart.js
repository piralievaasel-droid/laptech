...
export const cartPage = {
    ...
    `;
        const cartGrid = container.querySelector('.cart-grid');
      ...
      `;
            const paymentSelect = cartGrid.querySelector('#payment-method');
            const cardFields = cartGrid.querySelector('.card-fields');
        ...
        cartGrid.addEventListener('click', async (event) => {
            ...
            if (target.id === 'confirm-checkout') {
                const alertBox = container.querySelector('#checkout-alert');
                alertBox.textContent = '';
                const payment = (container.querySelector('#payment-method')?.value || 'Картой');
                const total = state.cart.reduce((sum, item) => sum + item.quantity * item.product.price, 0);
                if (!state.cart.length) {
                ...
                if (payment === 'Картой') {
                    const cardNumber = String(container.querySelector('#card-number')?.value || '').replace(/\s+/g, '');
                    const cardExpiry = String(container.querySelector('#card-expiry')?.value || '');
                    const cardCvc = String(container.querySelector('#card-cvc')?.value || '');
